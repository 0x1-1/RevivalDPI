; RevivalDPI Setup Script
; Inno Setup 6

#define MyAppName "RevivalDPI"
; Surum publish_release.ps1 tarafindan /DMyAppVersion=x.y.z ile gecirilir
; (kaynagi: kokdeki Directory.Build.props). Asagidaki deger yalnizca ISCC
; dogrudan elle calistirildiginda devreye girer.
#ifndef MyAppVersion
  #define MyAppVersion "1.6.0"
#endif
#define MyAppPublisher "RevivalDPI"
#define MyAppURL "https://github.com/0x1-1/RevivalDPI"
#define MyAppExeName "RevivalDPI.exe"
#define BuildDir "..\publish\RevivalDPI-win-x64"
...
turkish.LaunchAfterInstall=Kurulum tamamlandıktan sonra RevivalDPI'i çalıştır
...
english.LaunchAfterInstall=Launch RevivalDPI after installation
...
russian.LaunchAfterInstall=Запустить RevivalDPI после установки
...
AppId={{a28f7654-86e5-4b41-8a07-78dbf9e854e2}
AppName={#MyAppName}
...
InfoAfterFile=
OutputDir=..\dist
OutputBaseFilename=RevivalDPI-Setup-Windows-{#MyAppVersion}
SetupIconFile={#BuildDir}\res\revivaldpi.ico
...
Source: "{#PrereqDir}\VC_redist.x64.exe"; DestDir: "{app}\Prerequisites"; Flags: ignoreversion
Source: "{#PrereqDir}\Windows.Packet.Filter.3.6.1.1.x64.msi"; DestDir: "{app}\Prerequisites"; Flags: ignoreversion

[Icons]
Name: "{group}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"
Name: "{group}\RevivalDPI'i Kaldır"; Filename: "{uninstallexe}"
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; Tasks: desktopicon
Name: "{userappdata}\Microsoft\Internet Explorer\Quick Launch\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; Tasks: quicklaunchicon

[Registry]
Root: HKCU; Subkey: "SOFTWARE\RevivalDPI"; ValueType: string; ValueName: "InstallPath"; ValueData: "{app}"; Flags: uninsdeletekey
Root: HKCU; Subkey: "SOFTWARE\RevivalDPI"; ValueType: string; ValueName: "Version"; ValueData: "{#MyAppVersion}"; Flags: uninsdeletekey
Root: HKCU; Subkey: "SOFTWARE\RevivalDPI"; ValueType: string; ValueName: "Language"; ValueData: "{code:GetLanguageCode}"; Flags: uninsdeletekey
...
begin
  ...
  begin
    // MsgBox('.NET 8.0 Desktop Runtime, RevivalDPI için gereklidir.' + #13#10 + 
...
begin
  ...
  if RegQueryStringValue(HKCU, 'SOFTWARE\RevivalDPI', 'Language', LanguageCode) then
  begin
...
var
  ...
  RevivalDPIPath: String;
begin
  if CurStep = ssInstall then
  begin
    // Clean up localappdata\RevivalDPI folder before installation
    Log('=== Kurulum öncesi temizlik işlemi başlatılıyor ===');
    
    LocalAppDataPath := ExpandConstant('{localappdata}');
    RevivalDPIPath := LocalAppDataPath + '\RevivalDPI';
    
    if DirExists(RevivalDPIPath) then
    begin
      Log('localappdata\RevivalDPI klasörü bulundu, siliniyor...');
      try
        if DelTree(RevivalDPIPath, True, True, True) then
        begin
          Log('localappdata\RevivalDPI klasörü başarıyla silindi');
        end
        else
        begin
          Log('Uyarı: localappdata\RevivalDPI klasörü tamamen silinemedi (bazı dosyalar kullanımda olabilir)');
                        // Continue with installation even if cleanup is incomplete
        end;
      except
        Log('HATA: localappdata\RevivalDPI klasörü silinirken exception oluştu, kurulum devam ediyor');
      end;
    end
    else
    begin
      Log('localappdata\RevivalDPI klasörü bulunamadı, temizlik gerekmiyor');
    end;
...
begin
  ...
  begin
    ...
    // 2. Aşama: %localappdata%/RevivalDPI klasörünü sil (İlerleme: %40)
    Log('=== 2. AŞAMA: RevivalDPI klasörü siliniyor ===');
    try
      if Assigned(WizardForm) and Assigned(WizardForm.ProgressGauge) then
      begin
    ...
    begin
      if DelTree(RevivalDPIPath, True, True, True) then
      begin
        Log('RevivalDPI klasörü başarıyla silindi');
      end
      else
      begin
        Log('Uyarı: RevivalDPI klasörü silinemedi');
      end;
    end
    else
    begin
      Log('RevivalDPI klasörü bulunamadı');
    end;
    ...
    begin
      Log('Program Files\RevivalDPI klasörü siliniyor...');
      try
        if DelTree(AppPath, True, True, True) then
        begin
          Log('Program Files\RevivalDPI klasörü başarıyla silindi');
        end
        else
        begin
          Log('Uyarı: Program Files\RevivalDPI klasörü tamamen silinemedi (bazı dosyalar kullanımda olabilir)');
        end;
      except
        Log('HATA: Program Files\RevivalDPI klasörü silinirken exception oluştu');
      end;
    end
    else
    begin
      Log('Program Files\RevivalDPI klasörü bulunamadı');
    end;
    ...
    Log('Tüm kaldırma aşamaları tamamlandı. RevivalDPI kaldırılıyor...');
  end;
end;
